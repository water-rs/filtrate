// Helpers shared by the compute and fragment executions of a spatial stage.
//
// Untiled by design: spatial shaders read through the GPU texture cache with
// no workgroup shared-memory tiling, which benchmarks at the
// memory-bandwidth floor on tiler GPUs. Do not add `var<workgroup>` storage.
...
// coordinate. Spatial passes bind no sampler, so warps interpolate here
// instead of truncating to the nearest texel.
